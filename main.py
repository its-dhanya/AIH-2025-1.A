    ...
    try:
        ...
        if apply_semantic_filtering:
            try:
                ...
                semantic_model = SentenceTransformer('/app/models/all-MiniLM-L6-v2')
            except Exception as e:
...
def create_flat_outline(headings: List[Dict], title: str = None) -> Dict[str, Any]:
    ...
    Removes confidence, font_size, and is_bold.
    """
    outline_list = []
    ...
    for heading in headings:
        ...
        if 1 <= level <= 3: # Only include H1, H2, H3
            heading_item = {
                ...
                "text": heading["text"],
                "page": heading["page"],
                # Exclude 'confidence', 'font_size', 'is_bold' as per request
            }
...
def process_all_pdfs_in_directories(
    input_dir_path: str = "/app/input",
    output_dir_path: str = "/app/output",
